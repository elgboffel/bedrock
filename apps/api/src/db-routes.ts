/**
 * Database-backed API routes.
 *
 * These routes depend on the Drizzle DB service from the Effect context,
 * provided by DrizzleLive in the Layer stack. They're separated from
 * the non-DB routes so that unit tests (which don't spin up a database)
 * can test the other routes independently.
 *
 * The DB instance is yielded once in the outer Effect.gen and captured
 * via closure by each route handler. This is necessary because
 * effectRoute runs handlers with Effect.runPromiseExit (no context),
 * so handlers must have R = never.
 */

import { DB } from "@repo/database/client";
import { items } from "@repo/database/schema/index";
import { effectRoute } from "@repo/server/effect-route";
import { FastifyServer } from "@repo/server/fastify";
import { sql } from "drizzle-orm";
import { Effect } from "effect";

/**
 * Register database-backed routes on the Fastify instance.
 *
 * Requires both FastifyServer and DB in the Effect context.
 */
export const registerDbRoutes = Effect.gen(function* () {
  const app = yield* FastifyServer;
  const db = yield* DB;

  /**
   * GET /db/health -- verifies database connectivity.
   *
   * Runs a simple SELECT 1 query through the Drizzle client. If the
   * database is reachable, returns { status: "ok", timestamp }. If the
   * query fails, effectRoute catches the error and returns a 500.
   */
  app.get(
    "/db/health",
    effectRoute(() =>
      Effect.gen(function* () {
        yield* db.execute(sql`SELECT 1`);
        return { status: "ok" as const, timestamp: new Date().toISOString() };
      }),
    ),
  );

  /**
   * GET /db/items -- returns all items from the database.
   *
   * Uses Drizzle typed query API with the items schema.
   */
  app.get(
    "/db/items",
    effectRoute(() =>
      Effect.gen(function* () {
        return yield* db.select().from(items);
      }),
    ),
  );
});
